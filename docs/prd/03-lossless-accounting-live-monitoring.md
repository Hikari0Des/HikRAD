...
> Derived from [docs/PRD.md](../PRD.md) v1.1 on 2026-07-08 (updated 2026-07-09: FR-55 added, FR-36 gains the WhatsApp channel — Decision 16; updated 2026-07-11 for master v1.3: FR-60 infrastructure device monitoring added — Decision 23; updated 2026-07-18 for master Decision 41 — v2 phase 10: FR-89–90, customizable per-manager dashboards). Owns: FR-31, FR-32, FR-33, FR-34, FR-35, FR-36, FR-37, FR-38, FR-39, FR-40, FR-55, FR-60, FR-89, FR-90 · NFR-2 · Risks: lossless-pipeline complexity, competing with entrenched SAS4
> Depends on: [01-platform-install-licensing](01-platform-install-licensing.md) (Compose volumes for disk-backed queue, settings for retention/SMTP/Telegram/WhatsApp; **v2: FR-89/90 read/write FR-84's `manager_preferences` table**), [02-radius-nas-aaa](02-radius-nas-aaa.md) (accounting packet feed, CoA service, NAS registry), [05-billing-payments-vouchers](05-billing-payments-vouchers.md) (renewal/payment events for FR-55 receipts; **v2: FR-89's pending-payment-tickets widget reads `payment_tickets`**) · Depended on by: [04-subscribers-profiles](04-subscribers-profiles.md) (usage graphs on user page, quota counters), [05-billing-payments-vouchers](05-billing-payments-vouchers.md) (low-agent-balance alerts), [08-reports](08-reports.md) (usage report data)
...
### FR-89 (S) — v2: Dashboard widget catalog
**Master:** FR-32's one fixed tile set becomes a catalog of independently permission-gated widgets. A widget a manager lacks the permission for is never offered in the picker and never rendered.

*Elaboration:*
- **FR-89.1** — Every existing FR-32 tile becomes a catalog widget with a stable id, a permission requirement (a permission string per FR-27, `""` meaning "every authenticated manager, no permission needed"), and a size class. Two widgets are new this phase: **my-balance** (a manager's own balance card, self-view, no permission required — meaningful mainly for scoped/agent managers but not restricted to them, same "self-view is always open" posture as [01](01-platform-install-licensing.md) FR-84.2) and **pending payment tickets** (a count, gated on [05](05-billing-payments-vouchers.md) FR-79's `payment_tickets.verify`, reading `payment_tickets` directly the same way FR-32's revenue tile already reads `revenue_daily` directly — no cross-package Go import, a raw scoped SQL count). A third, **alerts feed** (recent alert events), is gated on this module's own `monitoring.view` and needs no cross-domain read at all.
- **FR-89.2** — Exact widget ids, their permission strings, and their size classes are frozen in `docs/v2/phases/phase-v2-10-custom-dashboards/00-phase.md`'s catalog table, not here — same split every prior v2 phase uses for exact shapes.
- **FR-89.3** — `GET /api/v1/dashboard` gains an optional `?widgets=<comma-separated ids>` query parameter. Omitted, it returns today's full aggregate (backward compatible — nothing existing breaks). When present, the response includes only the keys for ids the caller both requested **and** holds the permission for; a requested id the caller lacks permission for is silently dropped from the response — the whole call never fails with a 403 over one forbidden widget, matching FR-89's "absent, not erroring" rule. The server does the query work only for widgets actually present in the (permission-filtered) request, so a manager's reduced layout costs less than the full dashboard, not the same.

### FR-90 (S) — v2: Per-manager dashboard layout
**Master:** Each manager's dashboard order/visibility/size follows their account across devices; mobile always renders a single column.

*Elaboration:*
- **FR-90.1** — The layout (an ordered list of `{widget_id, size}`) is stored as one more optional key inside FR-84's `manager_preferences.doc` JSONB document — not a new table, not a migration. Absence of a stored layout means "today's fixed FR-32 dashboard, filtered to the widgets this manager can see" — the exact behavior every manager already gets, unchanged; this is the same "absence is the valid unset state" posture FR-84.1 already established for every other preference field.
- **FR-90.2** — Panel edit mode: add/remove from the permitted catalog only (a forbidden widget cannot be added even by editing a request by hand — the server-side permission filter in FR-89.3 is authoritative, the picker UI is a convenience, not the boundary), drag to reorder, a 1×/2× size toggle per widget, and reset-to-default. Reset-to-default is not a new endpoint: it is FR-84.2's existing full-document `PUT /me/preferences` with the layout key omitted, identical to how every other preference field already resets to unset.
- **FR-90.3** — Mobile always renders single-column (FR-32's phone-first requirement, unchanged) — the stored order still applies, only the column count collapses. A widget whose permission is later revoked (a role edit, an override) simply disappears from the manager's next-loaded layout without leaving a broken tile — the layout stores ids, never a snapshot of rendered content, so a missing permission just means that id is filtered out at render time exactly like it always was for the default layout.
