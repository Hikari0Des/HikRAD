...
> Derived from [docs/PRD.md](../PRD.md) v1.1 on 2026-07-08 (updated 2026-07-09: FR-58 added — Decision 19; WhatsApp opt-in field for [03](03-lossless-accounting-live-monitoring.md) FR-55 — Decision 16; updated 2026-07-16 for master v1.4: FR-61 subscriber `service_type` + FR-63 panel/UX added — Decision 28, v2 phase 1; updated 2026-07-18 for master Decision 39 — v2 phase 6: FR-85, subscriber email). Owns: FR-1, FR-2, FR-3, FR-4, FR-5, FR-6, FR-7, FR-8, FR-9, FR-10, FR-11, FR-12, FR-58, FR-61, FR-63, FR-85 · NFR-5
...
- **FR-1.6 — Delete, and when it is refused.** A subscriber can be deleted (`DELETE /api/v1/subscribers/{id}`, `subscribers.delete`, confirm-gated in the panel, available on the detail page and as a bulk action per FR-4.4) **only when they have no billing history**. If any `ledger_transactions` / `payments` / `card_payments` / `payment_intents` row references them, the delete is refused with `409 has_billing_history` and the panel says *"disable them instead, so their payments stay attached to them."*
  *Why refuse rather than allow:* `ledger_transactions.subscriber_id` and `payments.subscriber_id` are `ON DELETE SET NULL`, so deleting a paying customer leaves their money rows alive with **no owner** — revenue totals stay correct while the customer they came from vanishes from every per-subscriber report and receipt. `card_payments` / `payment_intents` are `NO ACTION` and would instead fail with a raw FK error. Neither is an acceptable answer to "remove this subscriber", and both are silent. The account an operator actually wants gone — a mis-typed row that never transacted — has none of these rows and deletes cleanly. This keeps the [05](05-billing-payments-vouchers.md) append-only ledger invariant intact: money is never edited or orphaned to make a UI verb work.
  Deleting a subscriber **does** remove their sessions/usage history and FR-64 NAS scopes (`ON DELETE CASCADE`); the audit entry is the only remaining trace, so it records the **username**, not just an id that no longer resolves.
...
### FR-4 (M) — Bulk actions
**Master:** on filtered user lists: enable/disable, change profile, extend expiry, move owner, delete, export CSV.

*Elaboration:* actions execute async with a progress/result summary (per-row failures listed), write one audit entry per affected user, and fire policy invalidation + CoA where state changed for online users. Export respects manager scoping and the `export` permission.

- **FR-4.1 — Target set: selection OR filter.** A bulk action runs against **either** an explicit set of ticked rows (`subscriber_ids`) **or**, when nothing is ticked, the **server-side filter** (the whole match set, not just the visible page). A non-empty selection **replaces** the filter; it is never unioned or intersected with it. *Why both:* an arbitrary set of rows cannot be expressed as a filter, so filter-only bulk made the commonest request ("disable these three") impossible and pushed operators into one-at-a-time edits or a filter they hoped matched nothing extra. The filter remains the right tool for "every expired subscriber". The panel **states which set is live** above the buttons — the two are one click apart and only one of them is undoable by hand. Added 2026-07-16 (owner-requested).
- **FR-4.2 — Manager scoping is never bypassed by a selection.** The scope clause applies on top of the ids, so a scoped agent listing another agent's subscriber id simply matches no row ([06](06-managers-roles-security.md) FR-27.2).
- **FR-4.3 — Select-all covers the rows on screen**, not the whole match set: ticking a box must never silently enlist rows the operator cannot see. "Everything matching the filter" is already expressible by ticking nothing.
- **FR-4.4 — Bulk `delete`** requires `subscribers.delete` (not `subscribers.edit`) — a bulk verb must never be a way around the permission its single-row equivalent enforces — and obeys FR-1.6's billing guard per row: a protected subscriber is reported as a per-row failure and skipped, never aborting the rest of the batch.
...
*Elaboration:* speeds stored as abstract down/up (+ optional burst per FR-11) — vendor VSA rendering is [02](02-radius-nas-aaa.md) FR-17's adapter job. Quota modes: unlimited | total bytes | separate down/up bytes, per billing cycle (reset on renewal). Edits prompt "apply to existing users now vs. on next renewal" (immediate apply fires policy invalidation, and CoA rate-change for online users via [02](02-radius-nas-aaa.md) FR-15.2).

- **FR-8.1 — Archive vs. delete.** **Archive** (`POST /api/v1/profiles/{id}/archive`, `profiles.edit`) hides a plan from the pickers while leaving it on the subscribers already using it — this is the answer for any plan that has ever been sold, because its price and speeds are what past renewals were sold *at*, and vouchers / payment history point at it. **Delete** (`DELETE /api/v1/profiles/{id}`, `profiles.delete`) is only for a plan **nothing has ever used**: any reference from `subscribers.profile_id` / `subscribers.pending_profile_id` / `vouchers` / `payment_intents` refuses it with `409 profile_in_use` and the panel says *"archive it instead"*. The DB's `ON DELETE RESTRICT` is the backstop; the pre-check exists so the refusal carries a reason rather than a raw FK error. *Why delete at all:* archive handles a retired plan well and a mistake badly — a plan created by typo, never sold, otherwise clutters the list forever, since "archived" still isn't "gone". Delete stays offered on an already-archived plan for exactly that reason. Wired 2026-07-16 (owner-requested; the permission existed unused since Phase 2).
...
### FR-85 (S) — Subscriber email (v2)
**Master:** `subscribers.email` — nullable, format-validated contact data alongside the existing phone/name, threaded through the panel form/list, portal self-edit, and CSV import.

*Elaboration:*
- **FR-85.1** — Data: `subscribers.email` nullable text, basic RFC-5322-shaped format validation (mirrors FR-1.3's phone-validation posture — reject clearly-malformed input client- and server-side, `422` with a `field_errors` entry on the offending field, never a silent drop). No uniqueness constraint (unlike username) — two subscribers sharing a household email is a real, unremarkable case. Storage is plain text, not encrypted — email is not a credential (contrast subscriber passwords, NFR-4/FR-1.1).
- **FR-85.2** — Surfaces: subscriber create/edit form gains an email field next to phone; the subscriber list gains an optional (column-chooser) email column; the portal's existing FR-44 self-edit screen gains email alongside phone/name/password; **FR-6's CSV import wizard** gains `email` as a mappable field (an SAS4 export commonly carries one — the shipped `sas4` preset maps a column named `Email` when present, same best-effort-default posture as every other preset field).
- **FR-85.3** — Every create/edit that touches email is audit-logged exactly like phone (FR-1.4) — before/after value, actor, timestamp.
- **FR-85.4 — Explicitly out of scope this phase.** Email is contact data only: no SMTP pipeline, no outbound receipt/alert delivery to it, no verification/confirmation flow (contrast FR-44's phone-confirmation flow, which this does **not** get). A future email notification channel — parallel to FR-36/FR-55's existing Telegram/SMTP/WhatsApp channels — is a candidate feature building on this column, not built here.
...
- **AC-85a** *(v2)* — Given a subscriber sets their email in the portal, then it shows on the panel's subscriber form; a malformed email is rejected `422` with a `field_errors` entry on `email` and writes nothing.
- **AC-85b** *(v2)* — Given a SAS4 CSV export with an `Email` column, when the dry run executes with the `sas4` preset, then the email column maps automatically and imports alongside the other fields; a row with a malformed email is reported as a per-row dry-run error, not written.

## 4. Data & interfaces

**Owned entities:** `subscribers` (username, password_enc, name, phone, **email** *(v2 FR-85, nullable)*, address, notes, owner_manager_id, profile_id, expires_at, status, mac_lock_mode, learned_mac, static_ip, overrides{rate, price}, pending_profile_id, **service_type** *(v2 FR-61, replaces `allow_hotspot`)*, whatsapp_opt_in, **nas_id / nas_service_id** *(v2 FR-64 assignment; nullable = any — columns enforced by [02](02-radius-nas-aaa.md))*), `profiles` (name, price, duration_days, rate_down/up, burst fields, hotspot_rate_down/up, quota_mode, quota bytes, throttle_rate, expiry_behavior, quota_behavior, pool_id, session_limit_default, archived, **nas_id / nas_service_id** *(v2 FR-64 default scope; nullable — enforced by [02](02-radius-nas-aaa.md))*), import batches/rows.
