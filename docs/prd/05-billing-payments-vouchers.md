...
- **v2-9:** `GET/PUT /api/v1/profiles/{id}/cost` (cost history read + new version write, admin/owner only, FR-71.2), `GET/POST /api/v1/overheads` (FR-73.1), `GET/POST /api/v1/reseller-prices` (FR-74.1, admin/owner only — a reseller never sets their own wholesale price), `GET /api/v1/reports/margin` (owner: full cost/wholesale/retail chain per FR-75.3; reseller-scoped: retail/their-wholesale only per FR-75.1/75.2).
