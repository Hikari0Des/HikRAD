...
6. **Monitoring UIs**: per-NAS status page (probe history charts — LTR inside RTL, downtime log); admin health page (FR-35: services, queue depth, **counter invariant badge**, disk); alerts: rules CRUD forms (type-specific threshold fields, channel routing, quiet hours), events list, in-app notification center fed by the notifications SSE + banner for critical (NAS down).
