...
- **C 0230–0239:** `health_probes` hypertable (nas_id?, device_id?, at, kind `icmp|snmp`, latency_ms, loss, cpu, mem, uptime, ok), `alert_rules` (type, threshold jsonb, channels jsonb, quiet_hours, cooldown_s, enabled), `alert_events` (rule_id, at, state, payload, deliveries jsonb), `monitored_devices` (name, ip, type `ap|switch|router|server|other`, snmp_community_enc?, location, notes, enabled — FR-60, amendment 2026-07-11).
...
- FR-60 (amendment 2026-07-11): `GET/POST/PUT/DELETE /api/v1/devices` (monitored-device CRUD, permission `monitoring.*`), per-device probe history via the same probe-history API shape as NASes; devices never join NAS lists/wizard/FreeRADIUS clients.
- `GET/POST/PUT /api/v1/alert-rules`; rule types frozen: `nas_down|nas_up|device_down|device_up|radius_reject_spike|acct_backlog|disk_low|expiring_digest|agent_balance_low` (`device_*` added 2026-07-11, FR-60); channels `inapp|telegram|email|whatsapp` (whatsapp added 2026-07-09 per Decision 16 — admin-alert recipients are WhatsApp numbers on the rule; creds/templates from settings, sub-PRD 01 FR-53.2; Phase 4 adds `push` and the FR-55 subscriber-facing messages). `GET /api/v1/alert-events` (paginated). In-app: SSE `GET /api/v1/live/notifications`.
...
8. FR-60: a monitored device (e.g. an AP simulated by any pingable host) goes unreachable → `device_down` alert fires and the devices health section shows it down; recovery fires `device_up`; the device appears nowhere in NAS screens or FreeRADIUS config.

## Execution efficiency (added 2026-07-11 — see protocol in [00-team.md](../00-team.md))
Scriptable gate items (2, 3 property/concurrency parts, 5 immutability tests, 7 data checks, 8 probe check) go into `scripts/gate-phase-3.sh` as agents complete them; the gate run = one session executing the script + the human checks (1, 4, 6 physical/UX legs). Agents load only their task file + cited contract sections.
...
*Amended 2026-07-11 (pre-start, Decision 23): FR-60 infrastructure device monitoring — `monitored_devices` schema (C1-C), device CRUD/probe APIs + `device_down|device_up` rule types (C5), gate item 8; backend agent-3, UI agent-5. See master PRD Decisions Log.*
