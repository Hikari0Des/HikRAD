...
Phase 2: AAA Core & Lossless Pipeline — a real subscriber authenticates, accounting is lossless, sessions are live on screen
  ...
  Agent 2 (RADIUS & NAS): NAS CRUD + RouterOS wizard snippets, CoA service, IP pools, vendor adapter, full authorize policy incl. dual-service, NAS discovery (FR-13–17, FR-58 enforcement, FR-56.1)
  Agent 3 (Accounting & Monitoring): lossless pipeline + sessions/usage hypertables (service-tagged) + reaper + audit counters + live WS (FR-31/33/37/38/40, NFR-2)
  Agent 4 (Backend Business): subscribers + profiles CRUD, search, bulk, auth read-model + policy rules incl. dual-service fields (FR-1–5, 7–10, FR-58 data)
...
Phase 3: Billing, Security & Monitoring — the MVP gate: renew-with-CoA, money ledger, roles/2FA, dashboard, alerts
  ...
  Agent 3 (Accounting & Monitoring): ICMP/SNMP health probes, alerts engine + channels incl. WhatsApp, dashboard API, system self-monitoring (FR-32, 34–36)
...
Phase 4: Portal, Payments & PWA — Noor renews at midnight from her phone
  Agent 1 (RADIUS & NAS): ROS 6/7 quirk matrix + CoA hardening + walled-garden entries + NAS API auto-setup preview/apply (FR-56.2–56.4)
  Agent 2 (Accounting & Monitoring): Web Push channel + expiring-soon digest + subscriber WhatsApp messaging + usage API polish (FR-36.3, FR-54.4 backend, FR-55)
  Agent 3 (Backend Business): portal API + PaymentGateway interface + mock adapter + first live adapter(s) + renewal event for FR-55 (FR-23, FR-41/42 backend)
...
Phase 5: Reports, Install & License — v1: installable in <30 min, provable zero loss, pilot-ready
  Agent 1 (Platform & Security): license system, backup/restore, update mechanism, install.sh + first-run wizard backend, optional Cloudflare tunnel, ops docs (FR-50/51, FR-49, FR-57)
