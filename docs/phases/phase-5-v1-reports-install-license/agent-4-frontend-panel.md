...
2. **Settings UI** (FR-53): grouped screens — locale (TZ/currency/formats/language), branding (logo upload with icon preview, colors), notifications (SMTP/Telegram with "send test" buttons), billing defaults (anchor rule, admin-balance bypass), backups (schedule/retention/path + last-backup age with staleness warning), data retention (with floor explanations), gateway enable/config (D's Phase-4 admin API). Admin-permission-gated.
