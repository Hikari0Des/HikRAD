...
4. **User detail page** (FR-3, key flow 2 step 2): status banner (online/offline live flag, expiry countdown, remaining quota), live session widget (SSE-driven), usage graphs daily/monthly (C7-C data; charts LTR inside RTL), session history table (incl. stale/reaped badges + last-disconnect reason + service pppoe/hotspot badge, FR-58), audit trail (A's endpoint), edit form (incl. the `allow_hotspot` toggle and `whatsapp_opt_in` consent toggle next to phone — FR-58.1/FR-1.5), **Reset MAC** one-click with confirm, disable-with-CoA-disconnect flow. Renew button present but disabled with "Phase 3" tooltip flag behind a feature switch. Profile form carries the optional hotspot rate fields (null = main rate).
5. **NAS screens** (FR-13/14): card list showing enabled state + last-seen (health colors arrive Phase 3 — leave the slot), create/edit wizard with type-specific steps, ROS 6/7 tabbed snippet block with copy button + "Test" (seen-since-created endpoint), delete-with-live-sessions confirm. **Discover** entry point (FR-56.1): button on the NAS list calling `POST /api/v1/nas/discover`, results list (identity/IP/ROS version, already-registered rows dimmed), pick one → wizard pre-filled.
