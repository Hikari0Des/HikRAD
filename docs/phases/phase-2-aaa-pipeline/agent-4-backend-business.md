...
## Tasks
1. Migrations 0100–0109 per phase C1-D (subscriber + profile columns) **plus** the frozen read-only SQL view `subscriber_quota_view` (subscriber_id, quota_mode, quota bytes, cycle anchor) consumed by C. [FR-1, FR-8]
...
8. AuthView per C4: single-query loader joining subscriber+profile+overrides+`quota:exhausted` flag (C8), sealed password passthrough, expired-pool name resolution; Redis cache with `LearnMac` write-through; benchmark hot path ≤ 1 ms, cold ≤ 20 ms. [C4, NFR-1 support]
