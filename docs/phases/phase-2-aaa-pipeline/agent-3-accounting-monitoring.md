...
3. Consumer group: stream → (a) dedup check via `acct_dedup` insert (conflict → `deduplicated++`, ack, done); (b) upsert `sessions` (Start creates; Interim updates counters/last_interim_at incl. 32-bit wrap + gigawords + counter-reset handling; Stop closes with cause; `service` derived from nas.type per amended C1-C — FR-58); (c) insert `usage_points` delta (with `service` tag); (d) update `live:sessions` Redis hash incl. computed rate (delta ÷ interval) + `service` field per amended C6; (e) evaluate quota state → write `quota:exhausted:<subscriber_id>` per C8 (read profile quota config via a read-only SQL view D freezes in C1-D — `subscriber_quota_view`); **quota math excludes `service='hotspot'` rows (FR-58.3)** but graphs/rollups include them; (f) DB-commit then XACK; `persisted++`. DB down → stop acking, stream grows, spill past memory threshold; drain in order on recovery. [FR-37.2/37.3, C8, FR-58 tagging]
...
6. Live feed: SSE `GET /api/v1/live/sessions` per C6 (snapshot + upsert/remove, filters, ScopeFilter via A's C2); `live.Count(subscriberID, service)`/`List` Go interface for B (per-service counting is what makes FR-58.2's +1-hotspot rule enforceable). Session history + usage endpoints per C7-C. [FR-31 backend, FR-33 API]
