...
- **FR-64 (S) — v2:** Subscriber/profile → NAS scoping, enforced at auth. Subscribers and profiles can be **assigned to specific NAS devices and service instances** (nullable = any NAS, the v1 behaviour). Assignment is **enforced at RADIUS auth time**: an assigned subscriber authenticating through a non-assigned NAS/service is rejected with a new reason `nas_not_allowed` (visible in the FR-39 debug tail, localized in the panel). Precedence: subscriber assignment overrides profile assignment; both empty = any NAS. IP pools follow the service instance (FR-62); the effective address is resolved static-IP → profile pool → service pool, and **when no HikRAD pool is set anywhere the authorize reply omits the `address_pool` intent** so the MikroTik falls back to its own locally-configured pool. Assignment changes fire `InvalidatePolicy` and are carried in the AuthView policy cache.
