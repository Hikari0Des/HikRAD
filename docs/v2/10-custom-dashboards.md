# v2-10 → phase v2-8 — Customizable per-manager dashboards
...
### FR-A — Widget catalog
- Every existing dashboard tile becomes a catalog widget with a stable id, a permission requirement, and a size class: online-now (+sparkline), revenue-today, RADIUS rps, subs active/expired/expiring, pipeline health, NAS health cards, plus cheap new ones surfaced by later phases (my-balance for agents, pending payment-review count once v2-7 lands, alerts feed).
...
- Layout doc (ordered widget list + size per widget) stored in `manager_preferences` (v2-4's JSONB store — hard dependency), versioned alongside the prefs schema; default layout = today's dashboard.
...
- Layout follows the account across devices (that is v2-4's job); localStorage only caches.
...
| `manager_preferences` | v2-4 | layout key in the versioned prefs schema |
...
```text
You are working in the HikRAD repo. We are starting v2 phase 8: customizable per-manager dashboards (PRD Decision 32; requires v2-4's manager_preferences to be shipped). You work SOLO — no parallel agents; execute sequentially (widget registry → layout store → edit mode → data-endpoint split), committing in reviewable chunks.

Read, in this order and nothing else yet: CLAUDE.md, docs/v2/phases/00-v2-execution-plan.md, docs/v2/10-custom-dashboards.md, the v2-4 phase brief + gate result (preferences schema/versioning), frontend/panel/src/pages/DashboardPage.tsx, and the dashboard aggregate endpoint it calls.
...
Step 2 — Create docs/v2/phases/phase-v2-8-custom-dashboards/00-phase.md with frozen contracts (widget catalog ids + permission map, layout JSON schema + version, endpoint split) and the integration gate (permission-gating test, cross-device layout test, default-equals-today snapshot; migrations only if the prefs schema version bumps — numbers are linear, next free). Scriptable items → scripts/gate-v2-phase-8.sh.
