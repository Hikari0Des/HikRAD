...
2. ▶ **Resolve service instance** via C7 (`vendor.For(nas.vendor).ResolveService(...)`) → `(service, *nasServiceRow)`. If multiple candidate instances of the coarse kind exist and none matches, reject (surfaced to FR-39). `service` from resolution supersedes the coarse hint.
...
### C10. Panel UX (FR-63)
Subscriber form: PPPoE/Hotspot/Both radio (persists `service_type`) + NAS/service assignment pickers (nullable = "Any NAS"). Profile form: NAS/service assignment pickers. NAS page: services sub-list (per-service status + session count) with per-service wizard steps. Subscriber list + live-sessions filter by `service_type`. New reject reason `nas_not_allowed` localized in the FR-39 debug reason map. **All new strings in `frontend/shared/locales/{en,ar,ku}/*.json`; `npm run i18n:check` CI-fatal.**
