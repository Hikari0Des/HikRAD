...
- **Socket:** `$HIKRAD_ROOT/data/updater/updater.sock` (created by the daemon on start, `0770`, owned by root — the directory itself is `mkdir -p` + nothing-special by `install.sh`, same idempotent-on-repair pattern as the `radius-control` socket dir; unlike that dir, no `chmod 777` is needed because only two parties ever need to reach this socket: the daemon that owns it and the one bind-mounted container — token auth is the real boundary, matching FR-86.2's "authenticated by a per-install shared token" as the actual security property, not filesystem permissions).
